...
function getHZ (){
    ...
    const Artikel = i+36;
    const Optionen = i+37;
    const Kategorie = i+24;
    const Arbeitsplatz = i+26;
    const Status = i+4;
    const Lieferzeit = i+31;
    const Gewicht = i+10;
    const Flaeche = i+12;

    let KategorieValue = 'Holzzuschnitt';
    let ArbeitsplatzValue = 'Holz Zuschnitt';
    let GewichtValue = '0';
    let FlaecheValue = '0';
    let LieferzeitValue = 14;

    t = document.getElementById("textfield-"+Artikel+"-inputEl").value.match(/[ßüÜäÄöÖa-zA-Z\s\-]+/)[0];
    o = document.getElementById("textareafield-"+Optionen+"-inputEl");
    ...
    g = o.value.match(/Kantenbearbeitung:\s(.*)/); k = g ? g[1]+' ' : '';
    
    
    bs = (b+'').replaceAll('.',',');
    ls = (l+'').replaceAll('.',',');

    x = b*l/10000; x += ''; FlaecheValue = x.replaceAll('.',',');
    x = b*l/10000*s*0.69 ; x += ''; GewichtValue = x.replaceAll('.',',');

    
    if(t.includes("Muster")){
        KategorieValue = 'Muster';
        ArbeitsplatzValue = 'Muster Holz';
        LieferzeitValue = 5;
        FlaecheValue = 0;
        GewichtValue = '0,07';
        r=t;
    ...
    } else if (t.includes("Floatglas")){
        KategorieValue = 'Floatglas';
        ArbeitsplatzValue = 'Verpackung ST';
        r=t + " " + s + " mm " + bs + " x " + ls + " cm, " + k;
    }else{
    ...
    setNumberField(Flaeche,FlaecheValue);
    setNumberField(Gewicht,GewichtValue);
    setNumberField(Lieferzeit,LieferzeitValue);
    setCombobox(Kategorie,KategorieValue);
    setCombobox(Arbeitsplatz,ArbeitsplatzValue);
    setCombobox(Status,'Aufgenommen');
